...
char *user_cnn_model_full_path(char *files, int index);//给定一个文件夹名称获取完整名称路径
...
int user_cnn_model_load_input_mnist(user_nn_list_matrix *mnist, user_nn_list_matrix *target, int mnist_index, user_cnn_layers *layers, int layers_index);//加载mnist数据
void user_cnn_model_ffp(user_cnn_layers *layers);//正向计算一次
void user_cnn_model_bp(user_cnn_layers *layers, user_nn_matrix *target, float alpha);//反向计算一次
