...
class Neurons:
    def __init__(self, dendrites, axon):
        self.dendrites = dendrites #神经元后级树突
        self.energy = 0  #神经元中心的能量值
        self.threshold = 0 #神经元中心的阈值
        self.axon = axon #神经元后级轴突

    def cell_tick(self):
        if sum(self.dendrites) > self.neurons_cell_threshold:
            self.neurons_cell_energy = sum(self.dendrites)
        self.neurons_cell_threshold = self.neurons_cell_energy**0.5
