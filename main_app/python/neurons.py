...
cell_blank_synapse = 0.1  # 树突无连接的数量比
...
class Networks:
    def __init__(self, cell_number, input_number, branch_length, cell_dendrite, axon_length, inhibit):
        ...
        self.dendrites_table = [randint(cell_dendrite, int(cell_dendrite * 1.5)) for _ in range(self.cell_num)]
        self.dendrites_table.append(input_number)  # 单个神经元拥有的树突表
        self.dendrites_number = int(sum(self.dendrites_table) * (1 + cell_blank_synapse))  # 计算总的树突个数
        self.cell_threshold_fix = [_*cell_threshold_max for _ in self.dendrites_table[0:-1]]  # 每个神经元的阈值
    ...
    def update_input(self, data):
        ...
        count_line = 0
        count_number = 0
        for index, count in enumerate(self.dendrites_table[0:-1]):  # 移动髓鞘数据到突触
            for v in self.axon_dendrites_list[count_line:count_line+count]:
                self.synapse[v][self.synapse_offset[count_number]] = self.axon[index][0]
                count_number += 1
            count_line = count_line + count
        biase = sum(self.dendrites_table[0:-1])
        for index in range(self.dendrites_table[-1]):  # 把输入数据移动到突触
            self.synapse[self.axon_dendrites_list[biase+index]][self.synapse_offset[biase+index]] = data[index]
        # for index, v in enumerate(self.axon_dendrites_list[-self.input_num:]):  # 把输入数据移动到突触
        #     self.synapse[v][self.synapse_offset[v]] = data[index]

    def update_output(self):
        self.axon = np.roll(self.axon, -1)  # 移动髓鞘数据
        count_line = 0
        for index, count in enumerate(self.dendrites_table[0:-1]):  # 通过树突更新髓鞘起始数据
