...
user_nn_hidden_layers *user_nn_layers_hidden_create(user_nn_layers *nn_layers,int feature_number) {
	...
	user_nn_matrix_init_vaule(hidden_layers->kernel_matrix, feature_number / 2, feature_number / 2);//初始化全连接的权重值
	user_nn_matrix_init_vaule(hidden_layers->biases_matrix, feature_number / 2, feature_number / 2);//初始化全连接的权重值
