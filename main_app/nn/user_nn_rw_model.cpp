...
static long user_nn_model_save_layer(FILE *file, long offset, user_nn_layers *layers){
	...
	fwrite(layers, sizeof(user_nn_layers), 1, file);//写入层
	return ftell(file);
}
//读取层信息
//file 文件对象
...
static long user_nn_model_read_layer(FILE *file, long offset, user_nn_layers *layers){
	fseek(file, offset, SEEK_SET);
	fread(layers, sizeof(user_nn_layers), 1, file);//写入层
	return ftell(file);
}
//保存输入层
//file 文件对象
...
static long user_nn_model_save_input(FILE *file, long offset, user_nn_input_layers *input){
	fseek(file, offset, SEEK_SET);
	fwrite(input, sizeof(user_nn_input_layers), 1, file);//写入层
	return ftell(file);
}
//读取输入层信息
//file 文件对象
...
static long user_nn_model_read_input(FILE *file, long offset, user_nn_input_layers *input){
	fseek(file, offset, SEEK_SET);
	fread(input, sizeof(user_nn_input_layers), 1, file);//写入层
	return ftell(file);
}
//保存卷积层
//file 文件对象
...
static long user_nn_model_save_output(FILE *file, long offset, user_nn_output_layers *output){
	fseek(file, offset, SEEK_SET);
	fwrite(output, sizeof(user_nn_output_layers), 1, file);//写入层
	return ftell(file);
}
//读取输出层
//file 文件对象
...
static long user_nn_model_read_output(FILE *file, long offset, user_nn_output_layers *output){
	fseek(file, offset, SEEK_SET);
	fread(output, sizeof(user_nn_output_layers), 1, file);//写入层
	return ftell(file);
}
//保存模型
//path 保存路径
...
bool user_nn_model_save_model(user_nn_layers *layers, int id){
	...
	while (1){
		...
		case u_nn_layer_type_null:
			layers_offset = user_nn_model_save_layer(model_file, layers_offset, layers);//保存层信息
			break;
		...
		case u_nn_layer_type_hidden:
			...
			layers_offset = user_nn_model_save_layer(model_file, layers_offset, layers);//保存层信息
			infor_offset = user_nn_model_save_hidden(model_file, infor_offset, hidden_infor);//保存卷积层数据
...
user_nn_layers	*user_nn_model_load_model(int id){
	...
	while (1){
		layers_offset = user_nn_model_read_layer(model_file, layers_offset, temp_cnn_layers);//获取层信息
		temp_cnn_layers->content = NULL;//避免内存重叠 清除内存同时清除了数据
		temp_cnn_layers->next = NULL;//避免内存重叠 清除内存同时清除了数据
		switch (temp_cnn_layers->type){
		case u_nn_layer_type_null:
			nn_layers = user_nn_layers_create(u_nn_layer_type_null, 0);//创建一个空层用于获取数据
			break;
		case u_nn_layer_type_input:
			temp_input_infor = (user_nn_input_layers *)malloc(sizeof(user_nn_input_layers));//分配层空间
			infor_offset = user_nn_model_read_input(model_file, infor_offset, temp_input_infor);//加载层信息
			input_infor = user_nn_layers_input_create(nn_layers, temp_input_infor->feature_width, temp_input_infor->feature_height);//创建输入层
			free(temp_input_infor);//释放空间
			break;
		case u_nn_layer_type_hidden:
			temp_hidden_infor = (user_nn_hidden_layers *)malloc(sizeof(user_nn_hidden_layers));//分配层空间
			infor_offset = user_nn_model_read_hidden(model_file, infor_offset, temp_hidden_infor);//读取输入层信息
			hidden_infor = user_nn_layers_hidden_create(nn_layers, temp_hidden_infor->feature_height);//创建卷积层
			...
			break;
		case u_nn_layer_type_output:
			temp_output_infor = (user_nn_output_layers *)malloc(sizeof(user_nn_output_layers));//分配层空间
			infor_offset = user_nn_model_read_output(model_file, infor_offset, temp_output_infor);//读取输入层信息
			output_infor = user_nn_layers_output_create(nn_layers, temp_output_infor->feature_height);//创建输出层
