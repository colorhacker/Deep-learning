...
void train_mnist_gen_network() {
	int gen_layers[] = { 'i',1,1,'h',196,'o',784 };
	...
	user_nn_matrix *start_kernel = user_nn_matrix_create(196,1);
	...
	if(train_kernel_matrces == NULL){ 
		//train_kernel_matrces = user_nn_matrices_create(1, 1000, 1, 196);
		for (int index = 0; index < 1000; index++) {
			user_nn_layers_all_delete(nn_gen_layers);//删除层
			nn_gen_layers = user_nn_model_load_model(1);//加载模型
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->height = 1;
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->width = 196;
			user_nn_matrix_cpy_matrix(start_kernel, ((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix);
			//user_nn_matrix_printf(debug_file, start_kernel);
			user_nn_matrix_printf(debug_file, ((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix);
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->height = 196;
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->width = 1;
			...
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->width = 196;
			user_nn_matrix_printf(debug_file, ((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix);
			user_nn_matrix_sub_matrix(((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix, ((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix, start_kernel);
			user_nn_matrix_printf(debug_file, ((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix);
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->height = 196;
			((user_nn_hidden_layers *)user_nn_layers_get(nn_gen_layers, 2)->content)->kernel_matrix->width = 1;
			fclose(debug_file);
