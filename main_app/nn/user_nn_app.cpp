	...
	while (1) {
		...
		for(int index_p = 0; index_p < parallel_count; index_p++){
			for (int index = 0; index < train_images->height * train_images->width; index++) {
				...
				loss_function = user_nn_model_return_loss(nn_layers[index_p]);
				if (sw_display) {
					user_nn_model_display_feature(nn_layers[index_p]);
				}
				if (loss_function <= loss_target) {
					user_nn_model_save_model(user_nn_model_nn_file_name, nn_layers[index_p]);//保存模型
					break;
				}
				if (save_model_count++ > 1000) {
					...
					user_nn_model_save_model(user_nn_model_nn_file_name, nn_layers[index_p]);//保存一次模型
					start_time = clock();
				}
			}
